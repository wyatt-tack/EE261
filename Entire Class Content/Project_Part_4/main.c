...
int main()
	...
	Location myLocation;
    char hemisphereLat, hemisphereLong;
	...
	while(readLine(fptr, lineArray) != -1)
		...
		if(strstr(lineArray, WANTED_RECORD)!= NULL
            ...
            sscanf(lineArray,
                    ...
                    dumpString, &myLocation.Latitude.degrees,
                    &myLocation.Latitude.minutes,
                    &myLocation.Latitude.seconds, &hemisphereLat,
                    &myLocation.Longitude.degrees,
                    &myLocation.Longitude.minutes,
                    &myLocation.Longitude.seconds, &hemisphereLong);
                //convert longitude/latitude to negative/positive values
                if (hemisphereLat == 'S')
                        myLocation.Latitude.degrees= -1*myLocation.Latitude.degrees;
                if (hemisphereLong == 'W')
                        myLocation.Longitude.degrees= -1*myLocation.Longitude.degrees;
                //convert decimal minutes to seconds
                myLocation.Longitude.seconds = 60 * myLocation.Longitude.seconds/100;
                myLocation.Latitude.seconds = 60 * myLocation.Latitude.seconds/100;
            //extract call sign from line data
			extractCallsign(lineArray, callSign);
			//print extracted data
			printf("%18s: Lat: %3d %02d %02d Long: %4d %02d %02d %06d records processed\n",
                callSign, myLocation.Latitude.degrees, myLocation.Latitude.minutes,
                myLocation.Latitude.seconds, myLocation.Longitude.degrees,
                myLocation.Longitude.minutes, myLocation.Longitude.seconds, packetCount);
			}
		}
